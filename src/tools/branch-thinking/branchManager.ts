...
└─────────────────────────────────────────────`;
  ...
  // Format for IDE chat output
  formatForIDEChat(branch: ThoughtBranch, context: ThinkingContext): string {
    const isActive = branch.id === this.activeBranchId;
    const currentThought = branch.thoughts[branch.thoughts.length - 1];
    
    if (!currentThought) {
      return 'No thoughts yet';
    }

    const secondsElapsed = Math.floor((new Date().getTime() - currentThought.timestamp.getTime()) / 1000);
    
    let output = '';
    
    // Include collapsible header - this emulates the "Thought for X seconds" section
    output += `Thought for ${secondsElapsed} seconds\n\n`;
    output += `${currentThought.content}\n`;
    
    if (currentThought.metadata.keyPoints.length > 0) {
      output += `\nKey points:\n`;
      currentThought.metadata.keyPoints.forEach((point, index) => {
